...
// Read Firebase client configuration safely
const firebaseConfigPath = path.join(process.cwd(), "firebase-applet-config.json");
const firebaseConfig = JSON.parse(fs.readFileSync(firebaseConfigPath, "utf8"));

// Initialize client-side Firestore instance running in server context
...
// Resilient API calling function with Key Rotation and Model Rotation Fallback
    ...
    for (const modelName of uniqueModels) {
      try {
        ...
        const timeoutPromise = new Promise<any>((_, reject) => {
          setTimeout(() => reject(new Error("Timeout (Gemini API did not respond within 30 seconds)")), 30000);
        });
...
// Local fallback visualizer trace generator (Runs when Gemini API is overloaded, rate-limited, or unavailable)
function generateFallbackTrace(rawTopic: string): any {
  ...
  // Magnificently formatted custom default list-processing template
...
async function startServer() {
  ...
  app.post("/api/progress/:userId", async (req, res) => {
    ...
    try {
      const docRef = doc(db, "user_progress", userId);
      await setDoc(docRef, { completedTopics }, { merge: true });
  ...
  // --- SAVED HISTORY / WALKTHROUGHS ENDPOINTS ---
  app.get("/api/history/:userId", async (req, res) => {
    const { userId } = req.params;
    if (!userId) return res.status(400).json({ error: "userId is required" });
    try {
      const docRef = doc(db, "user_history", userId);
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
  ...
  app.post("/api/history/:userId", async (req, res) => {
    ...
    try {
      ...
      savedWalkthroughs = savedWalkthroughs.filter((w: any) => w.id !== item.id);
      savedWalkthroughs.unshift(item); // Add to the front
      await setDoc(docRef, { savedWalkthroughs }, { merge: true });
  ...
  app.delete("/api/history/:userId/:itemId", async (req, res) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        savedWalkthroughs = savedWalkthroughs.filter((w: any) => w.id !== itemId);
        await setDoc(docRef, { savedWalkthroughs }, { merge: true });
        return res.json({ success: true, history: savedWalkthroughs });
      }
  ...
  // --- USER AI GENERATION STATS & RATE LIMITS ENDPOINT ---
  app.get("/api/generations/:userId", async (req, res) => {
    const { userId } = req.params;
    if (!userId) return res.status(400).json({ error: "userId is required" });
    try {
      const docRef = doc(db, "user_generations", userId);
      const docSnap = await getDoc(docRef);
  ...
  // AI-powered custom algorithm visualization generator (With server-authoritative daily rate limits & subscription checks)
  app.post("/api/visualize", async (req, res) => {
    ...
    // Server-Authoritative Limit Checking
    let currentCount = 0;
    const todayStr = new Date().toISOString().split("T")[0]; // YYYY-MM-DD
    let userGenRef = null;
    ...
    if (!isPro && userId) {
      try {
        userGenRef = doc(db, "user_generations", userId);
        const userGenSnap = await getDoc(userGenRef);
        if (userGenSnap.exists()) {
          const data = userGenSnap.data() as any;
          let count = data?.count || 0;
          let lastDate = data?.lastGenerationDate || "";
          if (lastDate === todayStr) {
            currentCount = count;
          } else {
            currentCount = 0; // Reset count on a new day
          }
        }
        ...
        // Fallback to allow if database error, but log it
      }
    }
...
Provide an extremely detailed, accurate trace so that clicking "Next" in the UI results in a smooth, logical, highly-polished learning animation.`;
      ...
      const response = await generateContentWithFallback({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              title: { type: Type.STRING, description: "Proper title of the algorithm" },
              description: { type: Type.STRING, description: "A concise description of what the algorithm accomplishes" },
              timeComplexity: { type: Type.STRING, description: "Worst-case time complexity, e.g. O(n log n)" },
              spaceComplexity: { type: Type.STRING, description: "Space complexity, e.g. O(1) or O(n)" },
              visualizationType: { 
                type: Type.STRING, 
                  ...
                  properties: {
                    elements: {
                      ...
                      items: {
                        type: Type.OBJECT,
                        properties: {
                          id: { type: Type.STRING, description: "Unique node/item ID" },
                          label: { type: Type.STRING, description: "Value to print inside the block" },
                          state: { 
                            type: Type.STRING, 
                          ...
                          index: { type: Type.INTEGER, description: "Index if array" },
                          nextId: { type: Type.STRING, description: "The node ID this node points to, for drawing connected arrows" },
                          x: { type: Type.INTEGER, description: "Optional x coordinate (0-100) for Tree/Graph layouts" },
                          y: { type: Type.INTEGER, description: "Optional y coordinate (0-100) for Tree/Graph layouts" }
                        },
                    ...
                    secondaryState: {
                      ...
                      properties: {
                        label: { type: Type.STRING, description: "Name of auxiliary structure, e.g. 'Seen Characters' or 'Recursion Stack'" },
                        type: { type: Type.STRING, description: "Structure type: set, map, stack, queue" },
                        items: {
                          type: Type.ARRAY,
                          items: {
                            type: Type.OBJECT,
                    ...
                    codeLine: { type: Type.INTEGER, description: "0-indexed line of code to highlight" },
                    explanation: { type: Type.STRING, description: "Narrative explanation of this state" }
                  },
      ...
      // If successful and not pro, save the incremented count to Firestore
      if (!isPro && userId && userGenRef) {
        ...
        visualData.remainingGenerations = 999; // Unlimited
      ...
      try {
        ...
        // Save the incremented count to Firestore even in fallback mode so that limits are respected
        if (!isPro && userId && userGenRef) {
          try {
            const newCount = currentCount + 1;
            await setDoc(userGenRef, {
        ...
        fallbackData.fallbackReason = err.message || "Gemini service unavailable";
  ...
  // --- START END-TO-END CODING SANDBOX ENDPOINTS ---
  const STANDARD_PRACTICE_QUESTIONS: Record<string, any> = {
    "p1": {
      id: "p1",
      title: "Two Sum",
      difficulty: "Easy",
  ...
  // Get question details with dynamic Gemini builder
...
Return a JSON object conforming exactly to this structure:
{
      ...
      "expectedOutput": "Expected output 3"
        ...
        config: {
          ...
          responseSchema: {
            ...
            required: [
              "title", "difficulty", "category", "description", "examples",
              "constraints", "starterCode", "timeLimit", "memoryLimit",
              "successRate", "submissions", "editorial", "hints", "testcases"
            ]
          }
        }
      });
  ...
  // End-to-end sandbox code executor running via dynamic Gemini execution environment
  app.post("/api/run-code", async (req, res) => {
    ...
    try {
      ...
      console.log(`Executing code via Gemini sandbox runner for ${language}...`);
      const response = await generateContentWithFallback({
        model: "gemini-2.5-flash",
...
Return a JSON response conforming EXACTLY to this schema:
{
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              compileError: { type: Type.STRING, nullable: true },
              runtimeError: { type: Type.STRING, nullable: true }
            },
  ...
  // --- END END-TO-END CODING SANDBOX ENDPOINTS ---

  // Serve static client assets in production; Vite in development
